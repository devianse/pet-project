...
Working notes from the initial scoping conversation (2026-08-07/08). Not a SPEC yet — repo/folder structure inside here and exact implementation phasing still TBD; this is the decision record so nothing discussed gets lost before that happens. See `../go-learning/knowledge-checks/LEARNING_PHILOSOPHY.md` and `../go-learning/knowledge-checks/GO_ROADMAP.md` for the learning context this grew out of.
...
Both are viable "barebones" options at this scale (2GB RAM comfortably runs all 3 containers). Not decided — Timeweb is the safer/simpler default (Russian company, no payment-method uncertainty, known-low latency); Cloudzy's crypto option is a fallback worth remembering if a domestic-card path turns out inconvenient. A domain name (needed for Caddy's automatic HTTPS) is a separate purchase either way, from neither of these.
...
- Exact repo/folder structure inside `pet-projects/` (deferred — "we will decide on repo structure later")
- VPS provider: Timeweb vs Cloudzy, final call — not needed until the shell is ready to deploy
- Which of Shopping List / Image Processing gets built first once the shell exists, or whether they're built in parallel
